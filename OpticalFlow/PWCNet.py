...
# end
    ...
    def forward_pyramid(self, tensorFirst, tensorSecond):
        ...
        tensorFlow_L3 = 5.0 * meta_flow
        tensorFlow_L2 = 2.0 * torch.nn.functional.interpolate(input=tensorFlow_L3, scale_factor=2, mode='bilinear', align_corners=False)
        tensorFlow_L1 = 2.0 * torch.nn.functional.interpolate(input=tensorFlow_L2, scale_factor=2, mode='bilinear', align_corners=False)

        # tensorFlow = 20.0 * torch.nn.functional.interpolate(
        #     input=self.forward_pre(tensorPreprocessedFirst, tensorPreprocessedSecond), size=(intHeight, intWidth),
        #     mode='bilinear', align_corners=False)

        tensorFlow_L1[:, 0, :, :] *= float(intWidth) / float(intPreprocessedWidth)
        tensorFlow_L1[:, 1, :, :] *= float(intHeight) / float(intPreprocessedHeight)

        tensorFlow_L2[:, 0, :, :] *= float(intWidth) / float(intPreprocessedWidth)
        tensorFlow_L2[:, 1, :, :] *= float(intHeight) / float(intPreprocessedHeight)

        tensorFlow_L3[:, 0, :, :] *= float(intWidth) / float(intPreprocessedWidth)
        tensorFlow_L3[:, 1, :, :] *= float(intHeight) / float(intPreprocessedHeight)

        return tensorFlow_L1, tensorFlow_L2, tensorFlow_L3
