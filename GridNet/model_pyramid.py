...
class GridNet(nn.Module):
    def __init__(self, in_chs=32, out_chs=3, grid_chs = [32, 64, 96]):
        ...
        self.lateral_init = LateralBlock( (in_chs+3)*2, self.n_chs[0])  # todo 
        self.lateral_init_2 = LateralBlock( 2*(self.n_chs[1]+0), self.n_chs[1])
        self.lateral_init_3 = LateralBlock( 2*(self.n_chs[2]+0), self.n_chs[2])
    ...
    def forward(self, input_level1, input_level2, input_level3):

        state_00 = self.lateral_init(input_level1)
        state_10 = self.down_0_0(state_00) + self.lateral_init_2(input_level2)
        state_20 = self.down_1_0(state_10) + self.lateral_init_3(input_level3)
